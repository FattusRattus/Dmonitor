...
def wifisniff(pk):
   if pk.haslayer(Dot11Deauth) | pk.haslayer(Dot11Disas):
      if str.upper(ap_bssid) in [str.upper(pk.addr2), str.upper(pk.addr1)]:
         ...
         if str([pk.addr2, pk.addr1, dmethod]) in list(database.keys()):
...
# Modify options to quote marks for spaces in ESSID
options.essid = '"' + options.essid + '"'
...
if ap_essid.replace('"', '') != options.essid.replace('"', ''):
...
if options.apdb != "":
   with open(options.apdb, 'r') as apfile:
...
print("========================================")
print("ESSID     : " + options.essid.replace('"', ''))
